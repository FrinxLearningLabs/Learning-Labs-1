...
* FRINX ODL is installed and reachable and you have installed the necessary features (check the installation basics if you are unsure)
...
Now we create the input and call "sync_from_network". This reads the most recent configuration from the two network devices. When you change a configuration on the device manually, this call will re-apply the configuration stored in FRINX ODL (database is master). If you prefer to always use the information from the network as single source of truth, use teh "checked_commit" call instead (network is master). 
...
Now we create the input and call "commit". This writes all changes to the network elements. If for some reason it is not able to apply the configuration to one or both devices, it will roll back both configurations. 
