...
**The second and third request add interface GigabitEthernet0/0/0/1 and GigabitEthernet0/0/0/3 to bundle 122**:
...
**Second and third requests add interface GigabitEthernet0/0/0/2 and GigabitEthernet0/0/0/5 to the bundle 187**:
...
After creating the configuration data for the network devices in config data store, we need to commit these changes and push them to the devices. Just before that, we can use the request called "CalculateDiff", that creates a list of changes we are about to commit:
